...
def main():
    ...
    for i in range(len(data)):
        coll = get_energized(data, direction=0, start=(i, 0))
        lava_produced.append(len(coll.keys()))
    for i in range(len(data)):
        coll = get_energized(data, direction=2, start=(i, len(data[0]) - 1))
        lava_produced.append(len(coll.keys()))
    for i in range(len(data[0])):
        coll = get_energized(data, direction=1, start=(0, i))
        lava_produced.append(len(coll.keys()))
    for i in range(len(data[0])):
        coll = get_energized(data, direction=3, start=(len(data) - 1, i))
        lava_produced.append(len(coll.keys()))
    print(max(lava_produced))

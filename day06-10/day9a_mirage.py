...
def get_next_element(array):
    difference = np.diff(array)
    if not difference.any():
        next_element = array[-1]
    else:
        next_element = array[-1] + get_next_element(difference)
    return next_element
...
def main():
    ...
    print(np.sum(np.apply_along_axis(get_next_element, 1, content)))
